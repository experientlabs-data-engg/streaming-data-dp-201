...
# Track offsets for all partitions
offsets = {}
...
def save_all_offsets():
    """Save offsets for all partitions to the offset file in MinIO."""
    try:
        # Convert offsets to JSON bytes
...
def start_offset_saver(interval):
    """Start a background thread to save offsets periodically."""
    def save_offsets_periodically():
        while True:
            save_all_offsets()
            threading.Event().wait(interval)  # Wait for the specified interval

    threading.Thread(target=save_offsets_periodically, daemon=True).start()
...
def consume_message():
    try:
        # Start the offset saver (e.g., every 5 minutes)
        start_offset_saver(interval=300)  # 300 seconds = 5 minutes
