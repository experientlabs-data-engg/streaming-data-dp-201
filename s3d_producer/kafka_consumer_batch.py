...
def save_all_offsets():
    """Save offsets for all partitions being consumed."""
    try:
        # Get the current assignment (partitions being consumed)
        assignment = consumer.assignment()
        offsets = {}

        for topic_partition in assignment:
            offsets[f"{topic_partition.topic}-{topic_partition.partition}"] = consumer.position(topic_partition)
        ...
        minio_client.put_object(
            ...
            OFFSET_FILE,
            json.dumps(offsets),
            len(json.dumps(offsets)),
        )
        logger.info("Saved offsets for all partitions.")
    except Exception as e:
...
def consume_message():
    try:
        # Calculate the end time (current_time - 5 minutes)
        ...
        while True:
            ...
            if msg.error():
                ...
                # Stop processing if the message timestamp is after the end time
                message_time = datetime.fromtimestamp(msg.timestamp()[1] / 1000)  # Convert timestamp to datetime
                if message_time >= end_time:
                    logger.info("Reached end time. Stopping consumer.")
                    break
        ...
        # Save offsets for all partitions before exiting
        save_all_offsets()
